...
namespace SMP.PL.Customers
{
    public partial class Customers_UserControl : UserControl
    {
        ...
        public Customers_UserControl(IUnitOfWork unitOfWork)
        {
            InitializeComponent();
            ...
            LoadData();
            IdList = new List<int>();
            _addForm = new AddForm(_unitOfWork);
        }
            ...
            if (result)
            {
                if (dataGridView1.RowCount > 0)
                {
                    ...
                    if (IdList.Count > 0)
                    {
                        ...
                        {
                            cs = await _unitOfWork._cusHelper.DeleteAsync(id);
                            ...
                            if (RowNo > 0)
                            {
                                ...
                                await _unitOfWork._systemHelper.AddAsync(new SystemRecords
                                {
                                    ...
                                    Details = "تم حذف العميل " + cs.Cus_Name,
        ...
        {
            loadingForm.Show();
            var data = await Task.Run(() => _unitOfWork._cusHelper.GetAll().ToList());
        ...
        {
            loadingForm.Show();
            var data = _unitOfWork._cusHelper.GetAll().ToList(); //await Task.Run(() => _unitOfWork._cusHelper.GetAll().ToList());
        ...
        private void Edit()
        {
            if (dataGridView1.RowCount > 0)
            {
                ...
                _addForm.SetCustomerId(RowId);
                _addForm.SetUserControl(this);
                _addForm.SetChangedData(dataGridView1.CurrentRow.Cells[1].Value.ToString());
                _addForm.Show();
            }
            else
            {
        ...
        {
            loadingForm.Show();
            dataGridView1.DataSource = await _unitOfWork._cusHelper.FindAsync(cus =>
                cus.Cus_Name.Contains(textBoxSearch.Text)
                || cus.Cus_Phone.Contains(textBoxSearch.Text)
                || cus.Cus_Email.Contains(textBoxSearch.Text)
            );
