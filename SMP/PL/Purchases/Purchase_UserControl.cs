...
{
    public partial class Purchase_UserControl : UserControl
    {
        ...
        public Purchase_UserControl(IUnitOfWork unitOfWork)
        {
            InitializeComponent();
            ...
            if (result)
            {
                if (dataGridView1.RowCount > 0)
                {
                    ...
                    if (IdList.Count > 0)
                    {
                        ...
                        {
                            pr = (await _unitOfWork._purHelper.FindAsync(p => p.Id == id)).ToList()[0];
                            pr.IsDelete = true;
                            await _unitOfWork._purHelper.EditAsync(id, pr);
                            ...
                            if (RowNo > 0)
                            {
                                ...
                                await _unitOfWork._systemHelper.AddAsync(new SystemRecords
                                {
                                    ...
                                    Details = "تم حذف عملية الشراء " + pr.Pur_Name,
        ...
        {
            loadingForm.Show();
            var data = await Task.Run(() => _unitOfWork._purHelper.GetAll().ToList());
        ...
        {
            loadingForm.Show();
            var data = _unitOfWork._purHelper.GetAll().Where(p => p.IsDelete == false).ToList();
        ...
        private void setColumnTitles()
        {
            ...
            dataGridView1.Columns[1].HeaderText = "اسم المنبج";
            dataGridView1.Columns[2].HeaderText = "النوع";
            dataGridView1.Columns[3].HeaderText = "الصنف";
            dataGridView1.Columns[4].HeaderText = "المورد";
            dataGridView1.Columns[5].HeaderText = "التفاصيل";
            dataGridView1.Columns[6].HeaderText = "سعر الشراء";
            dataGridView1.Columns[7].HeaderText = "سعر البيع";
            dataGridView1.Columns[8].HeaderText = "الكميه";
            dataGridView1.Columns[9].HeaderText = "سعر الشراء الكلى";
            dataGridView1.Columns[10].HeaderText = "سعر البيع الكلى";
            dataGridView1.Columns[11].HeaderText = "الربح";
        ...
        private void Edit()
        {
            if (dataGridView1.RowCount > 0)
            {
                ...
                _addForm.SetUserControl(this);
                _addForm.SetOldData(dataGridView1.CurrentRow.Cells[1].Value.ToString());
                _addForm.Show();
            }
            else
            {
        ...
        {
            loadingForm.Show();
            dataGridView1.DataSource = (await _unitOfWork._purHelper.FindAsync(
                pur => pur.Pur_Name.Contains(textBoxSearch.Text)
                || pur.Pur_Type.Contains(textBoxSearch.Text)
                || pur.Pur_Cat.Contains(textBoxSearch.Text)
                || pur.Pur_Supp.Contains(textBoxSearch.Text)
                || pur.Pur_Details.Contains(textBoxSearch.Text)
                || pur.Pur_Buy.ToString().Contains(textBoxSearch.Text)
                || pur.Pur_Sell.ToString().Contains(textBoxSearch.Text)
                || pur.Pur_Qt.ToString().Contains(textBoxSearch.Text)
                || pur.Pur_TSell.ToString().Contains(textBoxSearch.Text)
                || pur.Pur_TBuy.ToString().Contains(textBoxSearch.Text)
                || pur.Pur_TRev.ToString().Contains(textBoxSearch.Text)
            )).Where(pur => pur.IsDelete == false).ToList();
